...
const ModuleSettings = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:5421/api/v1/role_permissions');
        const data = response.data[0];

        const rolesData = data.role_permissions.map(rolePermission => ({ _id: rolePermission._id, role: rolePermission.role }));
        setRoles(rolesData);

        const moduleSettingsData = Object.keys(data.role_permissions[0].permissions);
  ...
  const handleSave = async () => {
    try {
      const { _id } = roles[activeRole];
      const permissions = Object.entries(toggleStates[_id] || {}).reduce((acc, [key, value]) => {
        if (value !== null) {
          acc[key] = { view: value };
        }
        return acc;
      ...
      <div className="grid grid-cols-4 gap-4 mt-4">
        {moduleSettings.map((setting) => (
          <div key={setting} className="flex items-center gap-2">
            <TriStateCheckbox
              value={toggleStates[roles[activeRole]?._id]?.[setting] || null}
              onChange={(e) => handleToggleChange(setting, e.value)}
            />
            <span>{setting}</span>
          </div>
        ))}
