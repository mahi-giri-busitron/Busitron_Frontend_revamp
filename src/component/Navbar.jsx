  ...
  return (
    ...
    >
      <div className="container mx-auto flex justify-between items-center">
        {/* Logo */}
        <Link to="/" className="text-2xl font-bold text-[#00715D] dark:text-white">
          MyLogo
        </Link>
            ...
            <li key={nav.id}>
              <ScrollLink
                ...
                className={`cursor-pointer hover:text-[#00715D] transition capitalize ${
                  scrolled || darkMode ? "text-white" : "text-gray-900"
                }`}
              >
                {nav.title}
              </ScrollLink>
        ...
        <div className="flex items-center space-x-6">
          ...
          >
            <div
              className={`absolute left-1 w-5 h-5 bg-white rounded-full shadow-md transform transition-all duration-300 ${
                darkMode ? "translate-x-6 bg-[#00715D]" : "bg-gray-800"
              }`}
            />
            <i
              className={`pi ${
                darkMode ? "pi-moon text-[#00715D]" : "pi-sun text-gray-800"
          ...
          {/* Login Button */}
          <button
            className="text-white bg-blue-600 px-5 py-2 rounded-lg font-semibold transition duration-300 ease-in-out hover:bg-blue-700 hover:shadow-lg cursor-pointer"
          >
            Login
          </button>

          {/* Mobile Menu Toggle */}
          <button onClick={() => setToggle(!toggle)} className="md:hidden cursor-pointer">
            <i className={`pi ${toggle ? "pi-times" : "pi-bars"} text-2xl text-[#00715D]`} />
          </button>
        </div>
      </div>
      ...
      >
        <div className="flex flex-col p-6">
          {/* Close Button */}
          <button onClick={() => setToggle(false)} className="self-end mb-4 cursor-pointer">
